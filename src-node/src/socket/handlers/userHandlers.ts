 ...
 *
 * @param {Namespace} io - The socket io namespace
...
const userHandlers = (io: Namespace<any>, socket: Socket, socketIdMap: { [userId: string]: string }) => {
    ...
    const confirmFollowRequest = async (requestId: number) => {
        ...
        io.to(socketIdMap[follow_request?.senderId as number]).emit("user:confirm_follow_request", JSON.stringify(follow_request?.receiver));
    }
